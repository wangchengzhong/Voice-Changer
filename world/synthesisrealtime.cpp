    ...
    WorldSynthesizer* synth, double* aperiodic_spectrum)
{
    ...
    int current_frame_ceil = static_cast<int>(ceil(current_location / synth->frame_period));
    double interpolation = current_location / synth->frame_period - current_frame_floor;
    ...
    WorldSynthesizer* synth)
{
    ...
    for (int i = 1 + synth->handoff; i < number_of_samples + synth->handoff; ++i)
        total_phase[i] = total_phase[i - 1] + 2.0 * world::kPi * interpolated_f0[i - synth->handoff] / synth->fs;
    ...
    if (number_of_pulses != 0)
        synth->last_location = synth->pulse_locations_index[pointer][number_of_pulses - 1];

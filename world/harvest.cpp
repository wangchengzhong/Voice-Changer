    ...
    double* f0_step4)
{
    ...
    for (int i = 0; i < number_of_boundaries / 2 - 1; ++i)
    {
        ...
        count = 1;
        for (int j = boundary_list[i * 2 + 1] + 1; j <= boundary_list[(i + 1) * 2] - 1; ++j)
    ...
    double* f0)
{
    ...
    int number_of_channels =
        1 + static_cast<int>(log(adjusted_f0_ceil / adjusted_f0_floor) / world::kLog2 * channels_in_octave);
    ...
    int y_length = static_cast<int>(ceil(static_cast<double>(x_length) / decimation_ratio));

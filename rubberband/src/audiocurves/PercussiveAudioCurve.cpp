...
float PercussiveAudioCurve::processFloat(const float *R__ mag, int)
{
...
double PercussiveAudioCurve::processDouble(const double *R__ mag, int)
{
    static double threshold = pow(10., 0.15);// 3dB rise in square of magnitude
    // DBG("thresholde: " << threshold);// always 1.41254
    ...
    const int sz = m_lastPerceivedBin;
    // DBG("sz:" << sz);// always 743 unless first few points 0 and fast grow to 743
    for (int n = 1; n <= sz; ++n) {
        double v = 0.0;
        if (m_prevMag[n] > zeroThresh)
        {
            v = mag[n] / m_prevMag[n];
            // DBG("v: " << v); // 0.5~2, almost 1, 743 times a frame
        }
        else if (mag[n] > zeroThresh)
        {
            v = threshold;
        }
        bool above = (v >= threshold);
    ...
    // DBG("nonzerosCount: "<< nonZeroCount);
    // at first is 0, then 38, 492, 719, 741, 742, 743, always 743

    // copy from mag to m_prevMag
    v_copy(m_prevMag, mag, sz + 1);

    if (nonZeroCount == 0) 
    {
        // DBG("return 0"); // only return at first several times
        return 0;
    }
    else
    {
        // DBG("return: " << double(count) / double(nonZeroCount));
        // often a double between 0.03~0.11
        // DBG("count of " << count << " amount of nonzero numbers: " << nonZeroCount);
        return double(count) / double(nonZeroCount);
    }
}

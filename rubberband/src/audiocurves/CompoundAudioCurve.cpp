...
void CompoundAudioCurve::setType(Type type)
{
    m_type = type;
}

void CompoundAudioCurve::reset()
{
    m_percussive.reset();
    m_hf.reset();
    m_hfFilter->reset();
...
void CompoundAudioCurve::setFftSize(int newSize)
{
...
float CompoundAudioCurve::processFloat(const float *R__ mag, int increment)
{
...
double CompoundAudioCurve::processFiltering(double percussive, double hf)
{
    ...
    {
        if (m_risingCount > 3 && m_lastResult > 0) 
            rv = 0.5;
        m_risingCount = 0;
    } 
    else

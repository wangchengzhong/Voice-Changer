...
const size_t RubberBandStretcher::Impl::m_defaultIncrement = 256;

const size_t RubberBandStretcher::Impl::m_defaultFftSize = 2048;

int RubberBandStretcher::Impl::m_defaultDebugLevel = 0;
    ...
    m_baseFftSize(m_defaultFftSize)
{
    ...
    // calling once before prepareToPlay, or each time you change the configuration
    // DBG("RubberBandStretcher::Impl::Impl: rate = " << m_sampleRate << ", options = " << options);
...
//    if (m_rateMultiple < 1.f) m_rateMultiple = 1.f;
    ...
    if ((options & OptionWindowShort) || (options & OptionWindowLong)) 
    {
        if ((options & OptionWindowShort) && (options & OptionWindowLong)) 
        {
            DBG("RubberBandStretcher::Impl::Impl: Cannot specify OptionWindowLong and OptionWindowShort together; falling back to OptionWindowStandard\n");
        } 
        else if (options & OptionWindowShort) 
        {
            ...
            if (m_debugLevel > -1) 
            {
                DBG("setting baseFftSize to " << m_baseFftSize << "\n");
            }
        } 
    ...
    if (m_options & OptionProcessRealTime)
    {
        ...
        {
            // DBG("have run here");// run here 2 times
    ...
    if (m_channels > 1) 
    {
        ...
        if (m_threaded && m_debugLevel > -1) 
        {
            DBG("Going multithreaded...\n");// never run here
        }
    }
#endif
...
void RubberBandStretcher::Impl::calculateSizes()
{
    ...
    if (m_realtime) 
    {
        ...
        else 
        {
            ...
            {
                // DBG("rsb is true");// never run here
                windowIncrRatio = 4.5;
            }
            else
            {
                // DBG("run here");// always run here when ratio>1 and change
                windowIncrRatio = 8;
            }
            ...
            while (outputIncrement > 1024 * m_rateMultiple &&
                   inputIncrement > 1) 
            {
                // DBG("run in strange range"); never run here
                outputIncrement /= 2;
                inputIncrement = int(outputIncrement / r);
            }
    ...
    if (m_debugLevel > 0) 
    {
        ...
        DBG("calculateSizes: analysis window size = " << m_aWindowSize 
            ...
            << m_fftSize << ", increment = " << m_inbufJumpSampleNum
            << " (approx output increment = " 
            << int(lrint(m_inbufJumpSampleNum * getEffectiveRatio())) << ")\n");
    }
...
//        windowSizes.insert(m_baseFftSize * 4);
    ...
    if (windowSizeChanged) 
    {
        ...
        m_aSincFilter = m_sincs[m_aWindowSize];
        ...
        m_realtime) 
    {
        for (size_t c = 0; c < m_channels; ++c) 
        {
            ...
            {
                rbs = m_inbufJumpSampleNum * 16;
            }
            // DBG(rbs); //8192 run 2 times
    ...
    m_stretchCalculator = new StretchCalculator
        (m_sampleRate, m_inbufJumpSampleNum,
...
void RubberBandStretcher::Impl::reconfigure()
{
    if (!m_realtime) {
        if (m_mode == Studying) {
...
void RubberBandStretcher::Impl::study(const float *const *input, size_t samples, bool final)
{
    ...
    size_t hasBeenPutInto_RB_inBufSampleNum = 0;
    ...
    while (hasBeenPutInto_RB_inBufSampleNum < samples) {

	size_t writableSampleNum = inbuf.getWriteSpace();
	writableSampleNum = min(writableSampleNum, samples - hasBeenPutInto_RB_inBufSampleNum);

	if (writableSampleNum == 0) {
            // warn
            cerr << "WARNING: writableSampleNum == 0 (hasBeenPutInto_RB_inBufSampleNum = " << hasBeenPutInto_RB_inBufSampleNum << ", samples = " << samples << ")" << endl;
	} else {
            inbuf.write(mixdown + hasBeenPutInto_RB_inBufSampleNum, writableSampleNum);
            hasBeenPutInto_RB_inBufSampleNum += writableSampleNum;
        }
	...
	while ((inbuf.getReadSpace() >= int(m_aWindowSize)) ||
	    ...
	    // available in m_inbuf.  We need to sendToFft m_aWindowSize of
	    // them for processing, and then skip m_inbufJumpSampleNum to
	    // advance the read pointer.
            ...
            inbuf.sendToFft(cd.accumulator, std::min(ready, m_aWindowSize));
            ...
            if (m_aWindowSize == m_fftSize) {
                ...
                // means we retain the m_aSincFilter cut if folding as well,
                ...
                if (m_aWindowSize > m_fftSize) {
                    m_aSincFilter->cut(cd.accumulator);
                }

                windowing_shiftingHalf_copyToDblbuf(tmp, m_fftSize, cd.accumulator, m_awindow);
                v_copy(cd.accumulator, tmp, m_fftSize);
            }

            m_studyFFT->forwardMagnitude(cd.accumulator, cd.fltbuf);

            float df = m_phaseResetAudioCurve->processFloat(cd.fltbuf, m_inbufJumpSampleNum);
            m_phaseResetDf.push_back(df);

//            cout << m_phaseResetDf.size() << " [" << final << "] -> " << df << " \t: ";

            df = m_stretchAudioCurve->processFloat(cd.fltbuf, m_inbufJumpSampleNum);
...
//            cout << df << endl;
            ...
            m_inputDuration += m_inbufJumpSampleNum;
//                cerr << "incr input duration by increment: " << m_inbufJumpSampleNum << " -> " << m_inputDuration << endl;
            inbuf.skip(m_inbufJumpSampleNum);
	}
    }
...
vector<int> RubberBandStretcher::Impl::getExactTimePoints() const
{
...
void RubberBandStretcher::Impl::calculateStretch()
{
    // DBG("have run here");
    ...
    // DBG("phase reset df mean = " << prdm << ", stretch df mean = " << sdm << "\n");
    // never run here
    ...
    if (m_outputIncrements.empty()) 
        m_outputIncrements = increments;
    else 
    {
        for (size_t i = 0; i < increments.size(); ++i) 
        {
...
size_t RubberBandStretcher::Impl::getSamplesRequired() const
{
    ...
    for (size_t c = 0; c < m_channels; ++c) {
        ...
        // DBG( "getSamplesRequired: ws = " << ws << ", rs = " << rs << ", m_aWindowSize = " << m_aWindowSize << "\n");
        // ws always 178, rs always 1891, m_aWindowSize always 2048
        ...
        if (ws == 0 && reqd == 0) reqd = m_inbufJumpSampleNum;
...
void RubberBandStretcher::Impl::process(const float *const *input, size_t samples, bool final)
{
    ...
    if (m_mode == JustCreated || m_mode == Studying) {
        ...
        if (m_mode == Studying) 
        {
            ...
            if (!m_realtime) 
            {
                ...
                if (m_debugLevel > 1) 
                {
                    DBG( "Not real time mode: prefilling\n");
                }
                for (size_t c = 0; c < m_channels; ++c) 
                {
                    m_channelData[c]->reset();
...
#ifndef NO_THREADING
        // just run when audio device changing
        if (m_threaded) 
        {
            MutexLocker locker(&m_threadSetMutex);
            ...
            // DBG(m_channels << "thread creating" << "\n");
    ...
    for (size_t c = 0; c < m_channels; ++c) 
    {
        hasBeenPutInto_RB_inBufSampleNum[c] = 0;
    }

    while (!allConsumed) 
    {

        // In a threaded mode, our "hasBeenPutInto_RB_inBufSampleNum" counters only indicate
        // the number of samples that have been taken into the input
        // ring buffers waiting to be processed by the process thread.
        // In non-threaded mode, "hasBeenPutInto_RB_inBufSampleNum" counts the number that
        ...
        for (size_t c = 0; c < m_channels; ++c) 
        {
            hasBeenPutInto_RB_inBufSampleNum[c] += consumeChannel(c,
                                          input,
                                          hasBeenPutInto_RB_inBufSampleNum[c],
                                          samples - hasBeenPutInto_RB_inBufSampleNum[c],
                                          final);
            if (hasBeenPutInto_RB_inBufSampleNum[c] < samples) 
            {
                allConsumed = false;
                //never run here
                //DBG("process: waiting on input consumption for channel " << c << "\n");
            } 
            else
            {
                if (final)
                {
                    m_channelData[c]->inputSize = m_channelData[c]->into_RB_InbufSampleCount;
                }
                // often run here
                // DBG("process: happy with channel " << c);
            }
            if (
#ifndef NO_THREADING
                !m_threaded &&
...
#endif
        // if (!allConsumed) DBG("process looping\n");
    }


    // DBG("process returning");
    // go here very often
    if (final) 
    {
        //DBG("final");// never run here
        m_mode = Finished;
    }
}
}

...
public:
     ...
     * samples actually read.
     */
    int sendToFft(T *const R__ destination, int n) const;
...
template <typename T>
int RingBuffer<T>::sendToFft(T *const R__ destination, int n) const
{
    int w = m_writer;
    int r = m_reader;
    //w:57~4091 random number from 0 to 4096
    // r:519~39..
    //DBG("w:" << w);
    //DBG("r:" << r);
    int available = readSpaceFor(w, r);
    ...
    {
        //never happen
        DBG("WARNING: RingBuffer::sendToFft: " << n << " requested, only "
            << available << " available\n");
	    memset(destination + available, 0, (n - available) * sizeof(T));
	    n = available;
    }
    if (n == 0) return n;
    //DBG("m_size:" << r); //m_size:4097, r: 0 to 4097
    int here = m_size - r;
    const T *const R__ bufbase = m_buffer + r;
    // DBG(*(m_buffer + r));
    if (here >= n) 
    {
        // DBG("run here:   " << here <<" n: " << n); //0 to 4096 vs 2048
        v_copy(destination, bufbase, n);
    } 
    else 
    {
        // DBG("run here"); // often run here
...
template <typename T>
int RingBuffer<T>::skip(int n)
{
    int w = m_writer;
    int r = m_reader;

    int available = readSpaceFor(w, r);
    // DBG(n << " available: " << available);
    if (n > available) 
    {
...
template <typename T>
template <typename S>
int RingBuffer<T>::write(const S *const R__ source, int n)
{
    int w = m_writer;
    int r = m_reader;

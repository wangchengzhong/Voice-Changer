...
#ifdef HAVE_KISSFFT
#include "D:/1a/voice_changer@wcz/VoiceChanger@wcz/rubberband/src/kissfft/kiss_fftr.h"
#endif
...
private:
    ...
    public:
        ...
        {
            // OutputDebugString("have run here!"); 
            // when using fft, we never
...
static std::string pickImplementation(int size)
{
...
std::set<std::string> FFT::getImplementations()
{
    ImplMap impls = getImplementationDetails();
...
std::string FFT::getDefaultImplementation()
{
    return defaultImplementation;
}
...
FFT::Precisions FFT::getSupportedPrecisions() const
{

    ...
    m_debugLevel(debugLevel)
{
    ...
    if (m_debugLevel > 0) 
    {
        DBG("Resampler::Resampler: using implementation: Speex with q = "
            << q << "\n");
        // only construct when initialize
    }
...
D_Speex::resample(float *const BQ_R__ *const BQ_R__ out,
                  ...
                  bool final)
{
    // DBG("have run here");// run here very often
    ...
    // DBG(incount << "   " << outcount);//incount: 256 
    // outcount: always 8192
    // incount 264/265/173(sometimes)/...
...
int D_Speex::resampleInterleaved(float *const BQ_R__ out,
                             int outcount,
                             const float *const BQ_R__ in,
                             int incount,
                             double ratio,
                             bool final)
{ 
    //interleaved: jiao cha cun qu de
    if (ratio != m_lastratio) 
    {
        setRatio(ratio);
    }
    ...
    if (final) {
        ...
        if (actual < expected) {
            ...
            if (final_in > 0) {
                ...
                {
                    // never happen here
                    // DBG("Resampler::Resampler: Speex resampler returned error "
                    //    << err<<"\n");
#ifndef NO_EXCEPTIONS
                    throw Resampler::ImplementationError;
#endif
...
D_Speex::reset()
{
    // DBG("run here"); // never have run here

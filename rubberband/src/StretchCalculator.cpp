...
void StretchCalculator::setKeyFrameMap(const std::map<size_t, size_t> &mapping)
{
...
std::vector<int> StretchCalculator::calculate(double ratio, size_t inputDuration,
...
void StretchCalculator::mapPeaks(std::vector<Peak> &peaks,
                                   ...
                                   size_t synthesisWindowSize)
{
    ...
    if (inIncrement == 0)
        inIncrement = m_inbufJumpSampleNum;
...
void StretchCalculator::calculateDisplacements(const std::vector<float> &df,
                                          float &maxDf,

...
TrackFooterComponent::TrackFooterComponent(EditViewState& evs, tracktion_engine::Track::Ptr t)
...
void TrackFooterComponent::valueTreeChildAdded(ValueTree& parentTree, ValueTree& c)
{
	if (c.hasType(te::IDs::PLUGIN))
		markAndUpdate(updatePlugins);
}

void TrackFooterComponent::valueTreeChildRemoved(ValueTree& parentTree, ValueTree& c, int indexFromWhichChildWasRemoved)
{
	if (c.hasType(te::IDs::PLUGIN))
		markAndUpdate(updatePlugins);
}

void TrackFooterComponent::valueTreeChildOrderChanged(ValueTree& parentTreeWhoseChildrenHaveMoved, int oldIndex, int newIndex)
{
	markAndUpdate(updatePlugins);
}
...
void TrackFooterComponent::mouseDown(const MouseEvent& event)
{

...
void AudioClipComponent::drawWaveform(juce::Graphics& g, tracktion_engine::AudioClipBase& c, tracktion_engine::SmartThumbnail& thumb, juce::Colour colour, int left, int right, int y, int h, int xOffset)
{
	...
	const bool usesTimeStretchProxy = c.usesTimeStretchedProxy();
	...
	else if (c.getLoopLength() == 0)
	{
		...
		drawChannels(g, thumb,
			...
			false, { t1,t2 }, c.isLeftChannelActive(), c.isRightChannelActive(),
			gainL, gainR);
	}
}

void AudioClipComponent::drawChannels(juce::Graphics& g, tracktion_engine::SmartThumbnail& thumb, juce::Rectangle<int> area, bool useHighRes, tracktion_engine::EditTimeRange time, bool useLeft, bool useRight, float leftGain, float rightGain)
{

...
public:
	...
	void process(FloatType* const buffer, const int bufferSize)
	{
		phaseVocoder.process(buffer, bufferSize, [&](FloatType* const buffer, const int bufferSize)
		{
			...
			for (int k = 5; k < N - 5; k++)
			{
				if ((spectrumAmp2[k] > spectrumAmp2[k - 1]) &&
					(spectrumAmp2[k] > spectrumAmp2[k + 1]) &&
					(spectrumAmp2[k] > spectrumAmp2[k - 2]) &&
					(spectrumAmp2[k] > spectrumAmp2[k + 2]) &&
					(spectrumAmp2[k] > spectrumAmp2[k - 3]) &&
					(spectrumAmp2[k] > spectrumAmp2[k + 3]) &&
					(spectrumAmp2[k] > spectrumAmp2[k - 4]) &&
					(spectrumAmp2[k] > spectrumAmp2[k + 4]))
				{
					peakIndex[peakNum] = k;
					peakNum++;
